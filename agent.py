...
def _run_tool_safe(name: str, tool_input: dict) -> str:
    """execute_tool 을 감싸 예외를 'Error: ...' 문자열로 변환한다.

    병렬 스레드에서 한 툴이 던진 예외가 다른 결과를 망치거나 루프를 깨지 않도록 격리한다.
    """
    try:
        return execute_tool(name, tool_input)
    except Exception as exc:  # noqa: BLE001 - 어떤 예외든 결과 문자열로 환원
        return f"Error: 툴 '{name}' 실행 중 예외: {type(exc).__name__}: {exc}"


def _execute_tool_blocks(blocks: list, approve=None) -> dict:
    """tool_use 블록들을 실행해 {block.id: 결과문자열} 를 반환한다.

    승인 게이트(REQUIRES_APPROVAL)는 인터랙티브 input() 이라 **메인 스레드에서 순차** 처리하고,
    승인된(또는 승인 불필요한) 툴만 병렬 실행한다. 파일/네트워크/subprocess 는 I/O 바운드라
    스레드로 실제 동시성이 난다(GIL 해제). "의존 없는 호출은 병렬로" — 지연을 줄이는 게 목적.
    """
    results = {}
    runnable = []
    for b in blocks:
        if b.name in REQUIRES_APPROVAL and approve is not None and not approve(b.name, b.input):
            results[b.id] = "Error: 사용자가 실행을 거부했습니다."
        else:
            runnable.append(b)

    if PARALLEL_TOOLS and len(runnable) > 1:
        with ThreadPoolExecutor(max_workers=min(len(runnable), MAX_PARALLEL_TOOLS)) as ex:
            fut_to_id = {ex.submit(_run_tool_safe, b.name, b.input): b.id for b in runnable}
            for fut, bid in fut_to_id.items():
                results[bid] = fut.result()  # 모든 작업 완료까지 대기
    else:
        for b in runnable:
            results[b.id] = _run_tool_safe(b.name, b.input)
    return results
